...
df = pd.read_csv("sheep_sample_information.txt", delimiter="\t", encoding='latin-1')

# Sadece "Sample" ve "Breed" sütunlarının seçilmesi
df_breed = df[["Sample", "Breed"]]
# Frekans dosyasının dataframe haline getirilmesi
df_freq = pd.read_excel("sheep_sample_information_breedFrequencies.xlsx")
...
#df_breed.to_excel('df_breed_sheep.xlsx', index=False)
#df_cleaned.to_excel('df_breed_sheep_cleaned.xlsx', index=False)
#df_cleaned.to_csv('breed_sheep_cleaned.txt', sep='\t', index=False)
...
ax.set_title('Sheep Genotype Frequencies by Genomic Locations')
...
#plt.show()
#plt.savefig('sheep_genotype_freq.png', dpi=300)