...
df = pd.read_csv("goat_sample_information.txt", delimiter="\t")

# Sadece "Sample" ve "Breed" sütunlarının seçilmesi
df_breed = df[["Sample", "Breed"]]
# Frekans dosyasının dataframe haline getirilmesi
df_freq = pd.read_excel("goat_sample_information_breedFrequencies.xlsx")
df_freq = df_freq.rename(columns={"Unnamed: 2": "Breed"})

#filtreleme işlemi ve NaN değerlerinin çıkarılması
df_freq = df_freq[["Breed","LSV (kg)", ]].dropna()

# Small_East_African_breed için LSV (kg) değerini al
small_east_lsv = df_freq.loc[df_freq['Breed'] == 'Small_East_African_breed', 'LSV (kg)'].values[0]

# Mashona_(Msh-1)_breed ve Mashona_(Msh-2)_breed için LSV (kg) değerini güncelle (aynı isme sahip oldukları belirtildiği için aynı LSV değeri uygulandı)
df_freq.loc[df_freq['Breed'] == 'Mashona_(Msh-1)_breed', 'LSV (kg)'] = small_east_lsv
df_freq.loc[df_freq['Breed'] == 'Mashona_(Msh-2)_breed', 'LSV (kg)'] = small_east_lsv
...
#df_breed.to_excel('df_breed_goat.xlsx', index=False)
#df_cleaned.to_excel('df_breed_goat_cleaned.xlsx', index=False)
#df_cleaned.to_csv('breed_goat_cleaned.txt', sep='\t', index=False)
...
"""output_file = "goat_genotip_frekanslari.txt"
...
ax.set_title('Goat Genotype Frequencies by Genomic Locations')
...
#plt.show()
#plt.savefig('goat_genotype_freq.png', dpi=300)